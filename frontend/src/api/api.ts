import { UserInfo, ConversationRequest, Conversation, ChatMessage, CosmosDBHealth, CosmosDBStatus, UserDetails, FrontendSettings } from "./models";
    ...
    const response = await fetch(`/history/list?offset=${offset}`, {
        ...
        const conversations: Conversation[] = await Promise.all(payload.map(async (conv: any) => {
            ...
            const conversation: Conversation = {
                id: conv.id,
                title: convMessages.length > 0 ? convMessages[0].content : conv.createdAt,
                date: conv.createdAt,
...
export const historyGenerate = async (options: ConversationRequest, abortSignal: AbortSignal, convId?: string, version?: string, ticketId?: string | null): Promise<Response> => {
    let body;
    if(convId){
        body = JSON.stringify({
            conversation_id: convId,
...
export const historyMessageAction = async (messageId: string, action: string): Promise<Response> => {
    const response = await fetch("/history/message_action", {
        method: "POST",
        body: JSON.stringify({
            message_id: messageId,
            action: action
