...
export function ChatHistoryPanel(props: ChatHistoryPanelProps) {
    ...
    const menuItems: IContextualMenuItem[] = [
        { key: 'clearAll', text: 'Clear all chat history'},
    ];
    ...
    return (
        <section className={styles.container} data-is-scrollable aria-label={"chat history panel"}>
            <Stack horizontal horizontalAlign='space-between' verticalAlign='center' wrap aria-label="chat history header" style={{borderBottom:'1px solid #ECECEC', marginTop:'10px'}}>
                <StackItem>
                    <Text role="heading" aria-level={2} style={{ alignSelf: "center", fontWeight: "600", fontSize: "18px", marginRight: "auto", paddingLeft: "26px", color:"#000000" }}>Chat History</Text>
                </StackItem>
                    ...
                    <Stack horizontal styles={commandBarButtonStyle}>
                        <CommandBarButton
                          ...
                          //  styles={commandBarStyle}
                            role="button"
                            id="moreButton"
                            className={styles.customchatData+" customhoverIcon"}

                            styles={{
                                root: { color: '#C2C2C2' },
                                icon: { color: '#C2C2C2', fontSize: 20 },
                                iconHovered:{color: '#C2C2C2', fontSize: 20} // Example icon-specific styling
                            }}
                        />
                        <ContextualMenu
                         styles={{
                            root:styles.contextualMenuRoot
                        }}
                            items={menuItems}
                            className={styles.clearChatStyle}
                        ...
                        <CommandBarButton
                            ...
                            // styles={commandBarStyle}
                            role="button"
                            className={styles.customchatData}
                            styles={{
                                root: { color: '#C2C2C2' },
                                icon: { color: '#C2C2C2', fontSize: 16 },
                                iconHovered:{color: '#C2C2C2', fontSize: 16} // Example icon-specific styling
                            }}
                        />
                    </Stack>
                </Stack>
